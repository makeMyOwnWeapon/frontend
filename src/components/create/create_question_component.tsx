...
// 클라→서버 : 토큰, 강의명, 문제, 문제팝업시간, 보기(객관식) or 빈칸(객관식), 정답, 해설
// 토큰, 문제, 문제팝업시간, 정답, 해설

const QuestionComponent: React.FC<{ id: number, onToggle: (id: number) => void, onDelete: (id: number) => void, expand: boolean, answers: string[], updateAnswer:(newAnswers:string[])=>void }> = 
({ id, expand, onToggle, onDelete, answers, updateAnswer  }) => {
      ...
      <QuestionContainer>
        ...
        {expand && (
          <>
            ...
            <InputBoxWrapper>
              <label>{questionType === 'objective' ? '문제: ' : '문제: '}</label>
