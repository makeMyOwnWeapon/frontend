...
const NicknameGenerator: React.FC<NicknameGeneratorProps> = ({ onNicknameGenerated }) => {
  ...
  const generateNickname = () => {
    const prefixes = [
      ...
      "알파", "우주의", "우주적인", "꿈의", "메아리치는", "불사조", "유령", "헤일로", "충돌의", "충격의",
      "전설의", "유성의", "성운의", "궤도의", "양자의", "균열의", "폭풍의", "왜곡의", "소용돌이치는", "선의의",
      "서풍의", "눈보라치는", "폭발의", "돌풍의", "안개의", "눈부신", "빛나는", "불타는", "불꽃의"
    ];
    const suffixes = [
      ...
      "오거", "악마", "천사", "아콘", "키메라", "지니", "인어", "마녀", "예언자", "현자",
      "순례자", "탐험가", "용병", "지휘관", "족장", "대장", "조종사", "우주비행사", "요원", "탐정",
      "수호자", "유목민", "방랑자", "약탈자", "반역자", "스파이", "과학자", "발명가", "학자", "사서",
      "궁수", "명사수", "치료사", "사제", "선원", "모험가", "장인", "대장장이", "농부", "상인",
      "광대", "시인", "미스틱", "마법부여사", "조각가", "화가", "음악가", "역사가", "철학자", "수학자",
      "엔지니어", "교사", "안내자", "정원사"
    ];
  ...
  return (
    <div>
      <button type="button" onClick={generateNickname} style={{ padding: '10px', borderRadius: '5px', backgroundColor: '#4CAF50', color: 'white', border: 'none', cursor: 'pointer', margin: '10px' }}>Generate Nickname</button>
      {nickname && <p style={{ fontWeight: 'bold' }}>{nickname}</p>}
    </div>
  );
};
